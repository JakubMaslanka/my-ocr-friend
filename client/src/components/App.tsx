...
function reducer(state: StateType, action: ActionType): StateType {
  switch (action.type) {
    ...
    case 'success':
      return { isConverting: false, convertingProgress: 100, ocrText: action.result };
    case 'reset':
...
function App() {
  const initialState = { isConverting: false, convertingProgress: 0, ocrText: null };
  const [state, dispatch] = useReducer(reducer, initialState);
  
  const intervalID = useRef(undefined) as React.MutableRefObject<number | undefined>;
  ...
  const handleConvert = () => {
    ...
    makeRequest({ imageUrl: imageRef.src }, (error) => addToast(`Internal server error, message: ${error}`, 'error'))
      .then((data) => {
        addToast('The photo was converted correctly!', 'success');
        dispatch({ type: 'success', result: data! });
      })
      .catch((err) => addToast(`Something went wrong, error message: ${err}`, 'error'));
  };
  ...
  const handleReset = () => {
    ...
    if (editableDiv.current!.hasChildNodes()) {
      Array
      .from(editableDiv.current!.children)
  ...
  const copyToClipboard = () => {
    if (ocrText) {
      ...
      navigator.clipboard.writeText(ocrText.result).then(() => {
  ...
  const openTextInTranslator = () => {
    if (ocrText) {
      window.open(`https://www.deepl.com/translator#en/pl/${encodeURIComponent(ocrText.result)}`, '_blank');
    } else {
      addToast('The result text is not converted yet!', 'danger');
    };
  };
  ...
  useEffect(() => {
    if (isConverting) {
      intervalID.current = window.setInterval(() => 
        dispatch({ type: "progressIncrease", percent: convertingProgress < 100 ? convertingProgress + 1 : 100 })
      , 250);
    }
      ...
      <div className="container mx-auto px-6 flex flex-col justify-between items-center relative py-4">
        <div className="flex flex-col">
          <h2 className="max-w-3xl text-2xl md:text-3xl font-bold mx-auto dark:text-white text-gray-800 text-center py-2">
            <span className="text-fuchsia-400">MY-OCR-FRIEND</span> is a web app allowing you to convert image into text.
            <br/>
          ...
          <div className="my-8">
            <label className="text-xs font-extrabold text-fuchsia-400 py-2" htmlFor="content">Paste image here:</label>
            <div
              id="content"
          ...
          {ocrText?.result && (
            <>
              <div className="flex flex-row justify-between items-center">
                <label className="text-md font-extrabold text-fuchsia-400 pt-2" htmlFor="content">Result:</label>
                <div className="flex flex-row gap-4">
                  <Tooltip tooltipText="Copy to clipboard">
                    <div onClick={copyToClipboard} className="bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 hover:bg-gray-300 transition-all duration-100 p-2 rounded-lg shadow-md cursor-pointer">
                      <span className="text-gray-800 dark:text-gray-200"><IoMdCopy /></span>
                    </div>
                  </Tooltip>
                  <Tooltip tooltipText="Translate text" >
                    <div onClick={openTextInTranslator} className="bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 hover:bg-gray-300 transition-all duration-100 p-2 rounded-lg shadow-md cursor-pointer">
                      <span className="text-gray-800 dark:text-gray-200"><MdOutlineTranslate /></span>
                    </div>
                  </Tooltip>
                </div>
              </div>
              <div className="max-w-3xl my-4 p-2 border border-fuchsia-500 shadow-lg outline-none relative">
                <span className="font-medium text-lg text-black dark:text-white">{ocrText.result}</span>
              </div>
